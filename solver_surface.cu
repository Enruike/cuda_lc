...
__global__ void relax_surf(const double* d_Qin_old, double* d_Qout, signed int* d_neighbor, unsigned int* d_Nvector_index, unsigned char* d_Nvector_signal, double* d_Qo, 
	...
	if (indx < surf) {
		...
		if (infinite == 0) {
			...
			for (int n = 0; n < 6; n++) {
				dQ[0][n] = surface_derivative_component(d_Qin_old, xm, xp, Qin[n], n, d_idx);
				dQ[1][n] = surface_derivative_component(d_Qin_old, ym, yp, Qin[n], n, d_idy);
				dQ[2][n] = surface_derivative_component(d_Qin_old, zm, zp, Qin[n], n, d_idz);
